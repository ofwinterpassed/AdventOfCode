...
int main(int argc, char** argv) {
  ...
  auto fixloop = [&](int ruleno) {
    auto& thisrule = rules[ruleno];
    if (thisrule.subrules.size() > 1) {
      auto found = find(thisrule.subrules[1].begin(), thisrule.subrules[1].end(), ruleno);
      if (found != thisrule.subrules[1].end()) {
        for (int a = 0; a < 10; ++a) {
          auto& cp = thisrule.subrules.emplace_back(thisrule.subrules.back());
          found = find(cp.begin(), cp.end(), ruleno);
          found = cp.erase(found);
          cp.insert(found, thisrule.subrules[1].begin(), thisrule.subrules[1].end());
        }
        for (auto& sr : thisrule.subrules) {
          found = find(sr.begin(), sr.end(), ruleno);
          if (found != sr.end()) {
            found = sr.erase(found);
            sr.insert(found, thisrule.subrules[0].begin(), thisrule.subrules[0].end());
          }
        }
      }
    }
