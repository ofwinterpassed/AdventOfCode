...
int main(int argc, char** argv) {
  ...
  for (string line; getline(cin, line);) {
    ...
    for (string line; getline(cin, line) && !line.empty();) {
      ...
      reast >>= 1;
      reast += (line.back() == '#') * 0x200;
      t.data.emplace_back(line.substr(1, line.size() - 2));
      swap(last, line);
    }
    t.data.pop_back();
    for (auto c : last) {
  ...
  vector<string> image(8 * side);
  for (int tid = 0; tid < grid.size(); ++tid) {

    auto getcoord = [&tiles](auto& tile, int j, int k) -> char {
      auto r = tile.rot;
      while (r > 0) {
        int oldj = j;
        j = k;
      ...
      if (tile.flip)
        k = 7 - k;
      return tiles[tile.tileIdx].data[j][k];
    };
    for (int j = 0; j < 8; ++j)
      for (int k = 0; k < 8; ++k)
        image[8 * (tid / side) + j].push_back(getcoord(grid[tid], j, k));
  }
  // clang-format off
  ...
  for (int i = 0; i < image.size() - 3; ++i)
    for (int j = 0; j < image.size() - 20; ++j) {
      bool found = true;
      for (int ii = 0; ii < 3; ++ii)
        for (int jj = 0; jj < 20; ++jj) {
          found &= (monster[ii][jj] != '#') || (image[i + 2 - ii][j + jj] == '#');
        }
      if (found)
        ++monsters;
    }
  ...
  int sumsquares = 0;
  for (auto& a : image) {
    sumsquares += count(a.begin(), a.end(), '#');
    clog << a << '\n';
  }

  clog << monsters << '\n';
  cout << sumsquares - 15 * monsters << '\n';
